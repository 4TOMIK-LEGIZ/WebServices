...
export class RegisterLawyerValidator {
    ...
    public async validate(
        ...
        const email: string = registerLawyerRequest.email.trim();
        if (email.length <= 0) {
        ...
        const user: UserTypeORM = await this.lawyerRepository.createQueryBuilder().where("email = :email", { email }).getOne();
        if (user != null) {
